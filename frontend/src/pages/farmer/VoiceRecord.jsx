...
function WaveVisualizer({ volume, isListening, isProcessing }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, height: 80, padding: '0 16px' }}
...
function MicButton({ isListening, isProcessing, onStart, onStop }) {
  const size = 120
  let bg, icon, label
  if (isProcessing) { bg = '#f59e0b'; icon = <Square size={44} color="#fff" />; label = 'Đang xử lý...' }
  else if (isListening) { bg = '#ef4444'; icon = <Square size={44} color="#fff" />; label = 'Nhấn để dừng' }
  else { bg = '#16a34a'; icon = <Mic size={44} color="#fff" strokeWidth={1.5} />; label = 'Nhấn để nói' }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <button onClick={isListening ? onStop : onStart} disabled={isProcessing}
        style={{
          ...
          boxShadow: isListening ? '0 0 0 12px rgba(239,68,68,0.15), 0 0 0 24px rgba(239,68,68,0.07)' : '0 6px 24px rgba(22,163,74,0.30)',
...
function CountdownBar({ isListening, maxSeconds = 30 }) {
  ...
  const color = seconds < 10 ? '#ef4444' : seconds < 20 ? '#f59e0b' : '#16a34a'
  return (
...
export default function VoiceRecord() {
  ...
  return (
    <div style={styles.page}>
      <style>{`
        @keyframes mic-pulse { 0%,100%{box-shadow:0 0 0 12px rgba(239,68,68,0.15),0 0 0 24px rgba(239,68,68,0.07)} 50%{box-shadow:0 0 0 16px rgba(239,68,68,0.20),0 0 0 32px rgba(239,68,68,0.10)} }
      ...
      <main style={styles.main}>
        <div style={{
          ...styles.textBox,
          borderColor: isListening ? '#bbf7d0' : hasResult ? '#16a34a' : '#e2e8f0',
          background:  hasResult ? '#f0fdf4' : '#f8fafc',
...
const styles = {
  ...
  errorBox:       { width: '100%', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '11px 14px', fontSize: 15, color: '#ef4444', lineHeight: 1.5 },
  infoBox:        { width: '100%', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 14px' },
  actionRow:      { display: 'flex', gap: 10, width: '100%' },
  btnRetry:       { flex: 1, padding: '13px', fontSize: 16, background: '#fff', border: '1.5px solid #e2e8f0', borderRadius: 12, cursor: 'pointer', color: '#0f172a', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 },
  btnConfirm:     { flex: 2, padding: '13px', fontSize: 17, background: '#16a34a', color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 },
