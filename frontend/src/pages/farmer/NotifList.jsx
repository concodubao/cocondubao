      ...
      {permission !== 'granted' && !isSubscribed && (
        <div style={styles.pushBanner} className="fade-up">
          <div style={styles.pushIconWrap}>
            <Bell size={18} color="#4B230A" strokeWidth={2} />
          </div>
          <div style={{ flex: 1 }}>
      ...
      <div style={styles.filterRow} role="tablist" aria-label="Lọc thông báo">
        {FILTERS.map(f => (
          <button key={f.key} role="tab" aria-selected={filter === f.key}
            ...
            style={{
              ...styles.filterBtn,
              background: filter === f.key ? '#4B230A' : 'transparent',
...
const styles = {
  page:           { minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: '#fdf8f5', fontFamily: "'Noto Sans', sans-serif", width: '100%', maxWidth: 480, margin: '0 auto', overflowX: 'hidden' },
  ...
  pushBannerBtn:  { padding: '7px 14px', background: '#4B230A', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap', boxShadow: '0 2px 6px rgba(22,163,74,0.3)' },
