...
function SessionCard({ session, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-start gap-3 bg-white border border-[#e5eeff] rounded-[20px]
                 px-4 py-4 shadow-[0_2px_8px_rgba(0,0,0,0.04)] active:scale-[0.99] transition-transform
                 text-left"
    >
      {/* Icon cây trồng */}
      <div className="w-11 h-11 rounded-2xl bg-[#f0fdf4] flex items-center justify-center flex-shrink-0 mt-0.5">
        <span className="material-symbols-outlined text-[22px] text-[#006b2c] ms-fill">{icon}</span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {session.preview ? (
          <p className="text-[13px] text-[#6e7b6c] leading-snug m-0 line-clamp-2">{session.preview}</p>
        ) : (
          <p className="text-[13px] text-[#c4cdc2] italic m-0">Phiên chat trống</p>
        )}
      ...
      <span className="material-symbols-outlined text-[18px] text-[#c4cdc2] self-center flex-shrink-0">
        chevron_right
      </span>
    </button>
  )
    ...
    <div className="min-h-dvh flex flex-col bg-[#f8f9ff] max-w-[480px] mx-auto">
      ...
      <header className="sticky top-0 z-10 flex items-center gap-2 px-4 py-3
        ...
        <button onClick={() => navigate('/chat', { state: {} })}
          className="flex items-center gap-1.5 h-9 px-4 bg-[#006b2c] text-white text-[13px]
      ...
      <main className="flex-1 flex flex-col gap-3 px-4 py-4">
        ...
        {!isLoading && sessions.length === 0 && (
          <div className="flex flex-col items-center gap-4 pt-16 text-center">
            ...
            <div>
              ...
              <p className="text-[13px] text-[#6e7b6c] mt-1">Hỏi Cò Con bất kỳ điều gì về cây trồng!</p>
            </div>
        ...
        {sessions.length > 0 && (
          <p className="text-center text-[12px] text-[#c4cdc2] pb-2 pt-1">
